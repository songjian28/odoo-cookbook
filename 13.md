...
代码发送一个Last-modified头及所生成的 HTML，告诉浏览器列表最后一次修改的时间。我们可以从library.book 模型的write\_date字段中提取这一信息。
...
如果在浏览器中访问 /my\_library/book\_details?book\_id=1，应该会看到ID为1的图书的详情页。如不存在，会收到一个报错页面。
