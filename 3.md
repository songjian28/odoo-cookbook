...
我们应当有一个包含\_\_manifest\_\_.py声明文件的模块来进行操作。你可能要按前一节的步骤来提供一个可以操作的模块。
...
> **📝**在向模块添加新文件时，不要忘记在\_\_manifest\_\_.py（数据文件）或\_\_init\_\_.py（代码文件）文件中进行声明，否则会忽略这些文件而不进行加载。
